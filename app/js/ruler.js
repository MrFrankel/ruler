...
var ruler = (function () {
  ...
  var constructGuide = function (dimension, x, y, e) {
    ...
    if (dimension === VERTICAL) {
      guide.style.left = ruler.utils.pixelize(x - options.container.offsetLeft);
    }
    else {
      guide.style.top = ruler.utils.pixelize(y - options.container.offsetTop);
    }
    guides.push({
      dimension: dimension,
      line: ruler.guideLine(guide, options.container.querySelector('.rul_wrapper'), dimension, options, curDelta, moveCB, e)
    });
  };
...
var setGuides = function (_guides) {
  _guides.forEach(function(guide){
    constructGuide(constructGuide(guide.dimension, guide.posX, guide.posY))
  })
